...
The built-in taxonomy (grouped food-COGS / beverage / operating-supplies) is seeded by migration `0018` and **duplicated in three places that must stay in sync** when the built-in list changes: `DEFAULT_CATEGORIES` in `public/static/utils.js` (frontend fallback + seed reference), the stat-chip/badge CSS in `public/static/style.css` (`.cat-<slug>` + `.cat-chip.cat-<slug>`, slug via `slugify`), and the backend `inferCategory` keyword rules in `src/index.ts` (auto-classifies invoice imports; unmatched → `Other`). The live picker reads from the `categories` table at runtime, not from `DEFAULT_CATEGORIES` (which is only the fallback). `Ingredients` is a retired legacy value kept for back-compat.
